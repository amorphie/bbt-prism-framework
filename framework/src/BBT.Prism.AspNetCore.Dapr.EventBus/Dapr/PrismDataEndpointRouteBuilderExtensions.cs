...
namespace Dapr
{
    ...
    /// </summary>
    public class PrismSubscribeOptions
    {
        /// <summary>
        ...
        /// </summary>
        public Func<List<PrismSubscription>, Task>? SubscriptionsCallback { get; set; }
    }

    /// <summary>
    /// This class defines subscribe endpoint response
    /// </summary>
    public class PrismSubscription
    {
        /// <summary>
        ...
        /// </summary>
        public PrismRoutes? Routes { get; set; }

        /// <summary>
        /// Gets or sets the metadata.
        /// </summary>
        public PrismMetadata? Metadata { get; set; }
    ...
    /// </summary>
    public class PrismMetadata : Dictionary<string, string>
    {
        /// <summary>
        /// Initializes a new instance of the Metadata class.
        /// </summary>
        public PrismMetadata() { }

        /// <summary>
        /// Initializes a new instance of the Metadata class.
        /// </summary>
        /// <param name="dictionary"></param>
        public PrismMetadata(IDictionary<string, string> dictionary) : base(dictionary) { }
    ...
    public class PrismRoutes
    {
        ...
        /// </summary>
        public List<PrismRule>? Rules { get; set; }
    }
...
namespace Microsoft.AspNetCore.Builder
{
    ...
    /// </summary>
    public static class PrismDaprEndpointRouteBuilderExtensions
    {
        /// <summary>
        ...
        public static IEndpointConventionBuilder MapPrismSubscribeHandler(this IEndpointRouteBuilder endpoints)
        {
        ...
        /// <returns>The <see cref="IEndpointConventionBuilder" />.</returns>
        /// <seealso cref="MapPrismSubscribeHandler(IEndpointRouteBuilder)"/>
        public static IEndpointConventionBuilder MapPrismSubscribeHandler(this IEndpointRouteBuilder endpoints,
            PrismSubscribeOptions options)
        {
            return CreateSubscribeEndPoint(endpoints, options);
        }

        private static IEndpointConventionBuilder CreateSubscribeEndPoint(IEndpointRouteBuilder endpoints,
            PrismSubscribeOptions? options = null)
        {
            ...
            return endpoints.MapGet("dapr/subscribe", async context => await SubscribeAsync(context, options));
        }
        ...
        private async static Task SubscribeAsync(HttpContext context, PrismSubscribeOptions? options = null)
        {
            ...
            var subscriptions = dataSource.Endpoints
                ...
                .Where(e => e.Metadata.GetOrderedMetadata<ITopicMetadata>()
                    .Any(t => t.Name != null)) // only endpoints which have TopicAttribute with not null Name.
                .SelectMany(e =>
                {
                    ...
                    var subs =
                        new List<(string PubsubName, string Name, string? DeadLetterTopic, bool? EnableRawPayload,
                            string Match, int Priority, Dictionary<string, string[]> OriginalTopicMetadata, string?
                            MetadataSeparator, RoutePattern RoutePattern)>();
                    ...
                    for (int i = 0; i < topicMetadata.Count(); i++)
                    {
                        subs.Add((topicMetadata[i].PubsubName,
                            ...
                            originalTopicMetadata.Where(m =>
                                    (topicMetadata[i] as IOwnedOriginalTopicMetadata)?.OwnedMetadatas?.Any(o =>
                                        o.Equals(m.Id)) == true || string.IsNullOrEmpty(m.Id))
                                .GroupBy(c => c.Name)
                ...
                .Select(e =>
                {
                    ...
                    var metadataSeparator = e.FirstOrDefault(e => !string.IsNullOrEmpty(e.MetadataSeparator))
                        .MetadataSeparator?.ToString() ?? ",";
                    var rules = e.Where(e => !string.IsNullOrEmpty(e.Match)).ToList();
                    var defaultRoutes = e.Where(e => string.IsNullOrEmpty(e.Match))
                        .Select(e => RoutePatternToString(e.RoutePattern)).ToList();
                    var defaultRoute = defaultRoutes.FirstOrDefault();

                    //multiple identical names. use comma separation.
                    var metadata = new PrismMetadata(e.SelectMany(c => c.OriginalTopicMetadata).GroupBy(c => c.Key)
                        .ToDictionary(c => c.Key,
                            c => string.Join(metadataSeparator, c.SelectMany(c => c.Value).Distinct())));
                    if (rawPayload || options?.EnableRawPayload is true)
                    {
                        metadata.Add(PrismMetadata.RawPayload, "true");
                    }
                    ...
                    if (logger != null)
                    {
                        ...
                        {
                            logger.LogError("A default subscription to topic {name} on pubsub {pubsub} already exists.",
                                first.Name, first.PubsubName);
                        }
                        ...
                        foreach (var entry in duplicatePriorities)
                        {
                            logger.LogError(
                                ...
                                first.Name, first.PubsubName, entry.Key, entry.Value);
                        }
                    }
                    ...
                    if (rules.Count > 0)
                    {
                        subscription.Routes = new PrismRoutes {
                            Rules = rules.Select(e =>
                                    new PrismRule { Match = e.Match, Path = RoutePatternToString(e.RoutePattern), })
                                .ToList(),
